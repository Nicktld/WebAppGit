...
async def select(sql, args, size=None):
    log(sql)
    global __pool
    async with __pool.acquire() as conn:## with-clause is equal to a function, hence use async to transform it a coroutine
        async with conn.cursor(aiomysql.DictCursor) as cur:
            await cur.execute(sql.replace('?', '%s'), args or ())## sql_placeholder-'?', mysql_placeholder-'%s'
            if size:
...
class BooleanField(Field):
    ...
    def __init__(self, name=None, default=False):
        super().__init__(name, 'boolean', False, default)## Boolean values cannot be primary key or other data types.
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def getValue(self, key):
        return getattr(self, key, None)# It still calls '__getattr__' method, the error can never be raised since a None would come out however.

    def getValueOrDefault(self, key):# Calls an unexisted attribute will initial itself with the default value.
    ...
    @classmethod # Usage: User.findAll
